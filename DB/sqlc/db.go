...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.createAccoutStmt, err = db.PrepareContext(ctx, createAccout); err != nil {
		return nil, fmt.Errorf("error preparing query CreateAccout: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.createAccoutStmt != nil {
		if cerr := q.createAccoutStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing createAccoutStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		createAccoutStmt:        q.createAccoutStmt,
